...
- **`apply_delta` path leaves orphan files when SQL rolls back** — `backend/src/grimoire/state_store/store.py:194-258` — `write_library_file` / `delete_library_file` mutate disk *before* opening `_txn`. If the SQL transaction rolls back, the file is gone (or wrongly present) and the index reflects the prior state — split-brain forever, repaired only by a full rescan.
- **`ImageGenService._results` and `_cache` grow without bound** — `backend/src/grimoire/imagegen/service.py:272-314` — Every seeded `GenerationResult` (which can include image bytes for `_inline_` results) is retained for the life of the service. No TTL, no LRU, `aclose()` doesn't clear. Long-running servers OOM.
