...
reference. Eleven stores hold scene ids: appearances (per-actor scenes lists),
...
is retired, the scene that ended it), rolls (per-entry scene field), prompt_log
...
keyed by scene id then post index), and alternates (a
...
from . import (alternates, changes, chronicle, commitments, commits, facts, plot,
               prompt_log, rolls, turnstate)
...
def repoint(cid: str, mapping: dict[str, str]) -> None:
    ...
    for mod in (alternates, appearances_paths, audit_baselines, changes, chronicle,
                commitments, commits, facts, plot, prompt_log, rolls, turnstate):
