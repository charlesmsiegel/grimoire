...
#: `.review.json` (#396) is two bytes longer again: an id that fits every
#: other sidecar and not that one is a scene whose absorb is refused after it
#: is minted, permanently, for a reason the reader cannot act on except by
#: renaming. `.steering.json` is the longest now — its writes are failsoft
#: (`steering.record` drops the row rather than failing the reroll), so an id
#: past it loses hints instead of features, but minting one on purpose would
#: be choosing that loss.
