...
same wire protocol/error-shape as openrouter.py (both build on
openai_wire.ChatCompletionsClient), but base_url is caller-supplied and the
API key is optional. See
...
from .openai_wire import ChatCompletionsClient, extract_error, status_kind
...
class OpenAICompatibleClient(ChatCompletionsClient):
    error_cls = OpenAICompatibleError

    def _headers(self, key: str) -> dict[str, str]:
        """Unlike OpenRouter the key is optional — a local endpoint may want no
    ...
    async def stream(self, messages, model: str, key: str, base_url: str,
        ...
        async for delta in self._stream_chat(url, key, payload):
            yield delta
    ...
    async def list_models(self, base_url: str, key: str) -> list[dict]:
        ...
        with self._normalized_errors():
            ...
            if resp.status_code >= 400:
                raise OpenAICompatibleError(status_kind(resp.status_code),
                                            extract_error(resp.text))
