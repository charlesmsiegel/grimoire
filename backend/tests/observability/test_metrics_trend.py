...
async def test_trend_empty_store(registry):
    buckets = await registry.trend("orchestrator", "turn", bucket="minute", window_seconds=600)
    assert buckets == []
...
async def test_trend_two_minute_buckets(registry):
    ...
    buckets = await registry.trend("orchestrator", "turn", bucket="minute", window_seconds=180)
...
async def test_trend_fills_empty_buckets_with_zeros(registry):
    ...
    buckets = await registry.trend("orchestrator", "turn", bucket="minute", window_seconds=240)
...
async def test_trend_hour_bucket(registry):
    ...
    buckets = await registry.trend("orchestrator", "turn", bucket="hour", window_seconds=7200)
    populated = [b for b in buckets if b["count"] > 0]
...
async def test_trend_rejects_out_of_range_window(registry):
    ...
    with pytest.raises(ValueError):
        await registry.trend("orchestrator", "turn", bucket="minute", window_seconds=30 * 86400 + 1)
...
async def test_trend_rejects_too_many_buckets(registry):
    ...
    # capped (>5000 buckets).
    with pytest.raises(ValueError):
        await registry.trend("orchestrator", "turn", bucket="minute", window_seconds=10 * 86400)
