...
async def test_check_contradictions_uses_top_k_search_and_judge(fact_factory):
    judge = ScriptedJudge(
        {
            (
                ...
                "winifred has never left Ashford County.",
            ): ContradictionVerdict.CONFLICT
        }
    )
    svc = ContinuityService(judge=judge)
    existing = fact_factory(text="winifred has never left Ashford County.")
    ...
    assert report.conflicts[0].existing_fact.text == "winifred has never left Ashford County."
...
OLD = "winifred has never left Ashford County."
