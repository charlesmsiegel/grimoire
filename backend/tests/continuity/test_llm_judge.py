...
async def test_judge_parses_conflict_verdict() -> None:
    ...
    existing = make_fact(fact_id="ex", text="winifred has never left Ashford.")
...
async def test_judge_integrates_with_continuity_service() -> None:
    ...
    existing = make_fact(text="winifred has never left Ashford.")
    await service.add_fact(existing, source="extractor")

    candidate = make_fact(
        fact_id="cand",
        text="winifred has never left Ashford.",  # same text → keyword overlap
    )
