    ...
    *,
    ...
    await store.db.execute(
        ...
        INSERT INTO turn_audits (
            turn_id, campaign_id, created_at
        ) VALUES (?, ?, ?)
        """,
        (turn_id, campaign_id, created_at.isoformat()),
    )
...
async def test_write_and_list_active_pin(store) -> None:
    ...
    active = await store.list_active_context_pins(campaign_id="camp")
    assert len(active) == 1
...
async def test_pin_with_ttl_expires_after_n_turns(store) -> None:
    ...
    active = await store.list_active_context_pins(campaign_id="camp", current_turn_id=turn_ids[0])
    assert len(active) == 1
    # Next sequential turn (1 elapsed) → still active even though `+` < digits
    # would lexicographically claim "expired".
    active = await store.list_active_context_pins(campaign_id="camp", current_turn_id=turn_ids[1])
    assert len(active) == 1, "TTL must use turn count, not lexicographic compare"
    # 2 elapsed → still under TTL of 3.
    active = await store.list_active_context_pins(campaign_id="camp", current_turn_id=turn_ids[2])
    assert len(active) == 1
    # 3 elapsed → TTL exhausted.
    active = await store.list_active_context_pins(campaign_id="camp", current_turn_id=turn_ids[3])
    assert active == []
    # 4 elapsed → still gone.
    active = await store.list_active_context_pins(campaign_id="camp", current_turn_id=turn_ids[4])
    assert active == []
...
async def test_pin_without_audit_history_treated_as_active(store) -> None:
    ...
    active = await store.list_active_context_pins(
        campaign_id="camp", current_turn_id="t_also_missing"
    )
    assert len(active) == 1
...
async def test_exclude_round_trip(store) -> None:
    ...
    rows = await store.list_active_context_pins(campaign_id="camp")
    assert len(rows) == 1
...
async def test_campaign_isolation(store) -> None:
    await store.write_context_pin(
        campaign_id="camp",
        kind="pin",
        target_entity_kind="character",
    ...
    other = await store.list_active_context_pins(campaign_id="other-camp")
    assert other == []
