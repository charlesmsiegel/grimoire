...
async def test_upgrade_setting_ref_refreshes_snapshots(store: StateStore) -> None:
    ...
    report = await store.upgrade_setting_ref(campaign_id="c1", setting_id="wod-london")
    assert "settings/wod-london/characters/winifred" in report.diff
    diff = report.diff["settings/wod-london/characters/winifred"]
