...
vi.mock("../api/client", async () => ({ ...await vi.importActual("../api/client"),
  api: { passageCharacterEvidence: vi.fn(), listCharacters: vi.fn(), draftPassageCharacter: vi.fn(), savePassageCharacter: vi.fn() } }));
const source = 'Mara said, "Wait."';
beforeEach(() => {
  vi.mocked(api.passageCharacterEvidence).mockResolvedValue({ mes_example: "<START>\n{{char}}: Wait.", quotes: ["Wait."] });
...
describe("reviewed passage character", () => {
  it("can save a sparse card without calling a model", async () => {
    const saved = show();
    await waitFor(() => expect(screen.getByRole("button", { name: "Create reviewed character" })).toBeEnabled());
    ...
    expect(api.savePassageCharacter).toHaveBeenCalledWith("realm", "scene", "response-a", {
      name: "Mara", passage: source, source_text: source, description: "", mes_example: "<START>\n{{char}}: Wait." });
  });
