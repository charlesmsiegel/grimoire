...
const UPDATE = {
  ref: { kind: "lore", id: "the-tide-pact" }, status: "update",
  world: { name: "The Tide Pact", body: "Signed at dawn." },
  mine: { name: "The Tide Pact", body: "Signed at dawn." },
};
...
test("the three systems fold into one row per ref, not three lists", async () => {
  ...
  (api.listDiverged as any).mockResolvedValue([
    { ref: { kind: "items", id: "the-grey-key" }, name: "The Grey Key" }]);
  ...
  expect(screen.getByRole("button", { name: /Saltmarch Harbour/ }).textContent)
    .toContain("conflict");
  expect(screen.getByRole("button", { name: /The Tide Pact/ }).textContent)
    .toContain("update pending");
  expect(screen.getByRole("button", { name: /The Grey Key/ }).textContent)
...
test("a row's detail sends the review to that ref", async () => {
  (api.getIncoming as any).mockResolvedValue([CONFLICT, UPDATE]);
  await renderPanel();
  fireEvent.click(screen.getByRole("button", { name: /The Tide Pact/ }));
  await act(async () => {});
  ...
  expect(onReview).toHaveBeenCalledWith({ kind: "lore", id: "the-tide-pact" });
});
...
test("a diverged record has nothing to review, and is told where its moves live", async () => {
  (api.listDiverged as any).mockResolvedValue([
    { ref: { kind: "items", id: "the-grey-key" }, name: "The Grey Key" }]);
  await renderPanel();
  fireEvent.click(screen.getByRole("button", { name: /The Grey Key/ }));
  await act(async () => {});
  expect(screen.getByRole("button", { name: "See the change" })).toBeDisabled();
...
test("a failed read is reported, because 'nothing outstanding' is the wrong answer", async () => {
  ...
  expect(screen.getByRole("button", { name: /The Tide Pact/ })).toBeInTheDocument();
});
...
test("the empty body says what this view cannot enumerate yet", async () => {
  ...
  expect(screen.getByText(/Records this campaign\s+follows with nothing pending are not listed/))
    .toBeInTheDocument();
});
