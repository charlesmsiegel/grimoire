...
starts the existing FastAPI app on `127.0.0.1:<port>` (a port drawn once per install and
kept, falling back to one the OS assigns), and renders the existing
...
| 3 | **`Pillow` wheel (C).** | Low | In Chaquopy's official repo, built without libwebp: it ships the WebP plugin's Python but not its `_webp` module, so it can neither write WebP nor read it. Thumbnails (`store/thumbs.py`) ask Pillow what it can write and use JPEG there, or PNG where a picture has alpha, in a cache generation of their own beside a desktop's WebP one on a synced library; a stored WebP image cannot be decoded, so it is served as its original. `test_thumbs.py` runs the fallback against a Pillow with `_webp` blocked. |
| 4 | Remaining deps (`httpx`, `jinja2`, `holidays`, `pyluach`, `certifi`, `python-multipart`, `uvicorn` sans extras) | None | Pure Python. |
| 5 | Cold start: interpreter + FastAPI import on mid-range hardware. | Medium | The server binds and reports its port *before* the imports (`android_entry.py`), so the WebView's first request waits in the listen backlog while they run; the loading screen stays up until that first page finishes loading (`onPageFinished`). One known gap: an import that fails after the port was reported resets the queued connection, so the player sees the WebView's error page rather than the loading screen's status text (the message still reaches logcat). Lazy imports are gone — every import in `backend/src/grimoire/` is at module scope (`backend/tests/test_import_guard.py`). `tiktoken` and `claude_agent_sdk` stay absent on Android and cost nothing: each is guarded by a module-level sentinel (`store/tokens.py`, `claude_agent.py`). `jinja2` is a base dep, so its import cost now lands when `grimoire.prompts` is imported rather than at first render — the one cold-start cost this buys. Budget: ≤2.5 s cold on a mid-range device; measure in Phase 0. |
